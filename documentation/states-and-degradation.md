...
| **Model names an unsafe aroma material** | `material` or `title` matches the blocked list (controlled substances, household chemicals and fuels, poisonous plants) | the item is dropped in `CATEGORIES["aroma"]["postprocess"]`; **no top-up call** — a selection of 4–6 survives losing one | One item fewer, silently | — (automatic); if `dropped_unsafe` in the event keeps filling, the prompt needs fixing, not the filter |
| **Aroma item from before 2026-08-12** | `material`/`form` absent — no migration was run | frontend renders the item without the "form · material" line; `avoid_key` falls back to `title` | The item looks exactly as it did before | Regenerate the atmosphere for that book |
