...
class User(SQLModel, table=True):
    ...
    # ⚠ С задачи 124 (06.08) поле НЕ используется: свободный текст заменён
    # явными настройками ниже. Колонка оставлена, чтобы правка была
    # обратимой; удаление — з.125.
    wishes: Optional[str] = None
    # --- Настройки рекомендаций (задача 124, миграция 0020) ---
    # Не советовать авторов, которые уже есть на полке: про знакомого автора
    # читатель сам знает, что хочет у него дальше.
    rec_skip_known_authors: bool = False
    # Жанры «хочу» и «не хочу» — списки `Genre.slug` через запятую.
    # ⚠ Именно slug, а не id: жанр без книг удаляется (`_drop_orphans`),
    # и ссылка по id протухла бы молча. По slug максимум ничего не найдётся.
    rec_genres_include: Optional[str] = None
    rec_genres_exclude: Optional[str] = None
...
class AISelection(SQLModel, table=True):
    ...
    verified: bool = True
    # з.99 (02.08): рассуждение модели, которое reasoning-as-schema заставляет
    # заполнить ДО ответа (интонация, кухня, свежие исполнители). Раньше
    # выбрасывалось: в payload уезжали только songs/items, и проверить, сработал
    # ли приём, было нечем. Пользователю не показывается — это материал для
    # разбора качества (з.84/87) и для замеров вроде scripts/explore_avoid.py.
    analysis: str = ""
    created_at: datetime = Field(default_factory=datetime.now)
...
# --- Автор (задача 97). Общая сущность, как книга и цикл: автор существует
# объективно, независимо от того, кто его читает.
#
# Почему отдельная таблица, а не строка `Book.author`: строка не даёт страницы
# автора, склеивает соавторов в одно имя («Екатерина Казакова, Алена Харитонова»)
# и рассыпает статистику при малейшем расхождении в написании.
#
# Два имени вместо одного — решение Ксении: в библиотеке 148 авторов записаны
# кириллицей и двое латиницей (Ann Patchett, Joan Didion). Без второго поля они
# либо выглядят инородно в списке, либо теряют оригинальное написание.
#
# ⚠ `Book.author` НАМЕРЕННО остаётся на месте: это строка для показа, печатной
# карточки и обратной совместимости API. Связи добавляются РЯДОМ, а не вместо —
# миграция неразрушающая, откат ничего не теряет.
class Author(SQLModel, table=True):
    id: Optional[int] = Field(default=None, primary_key=True)
    name_ru: Optional[str] = None          # основное имя для показа
    name_original: Optional[str] = None    # написание на языке оригинала
    # Ключ тождества: нормализованное имя (регистр, лишние пробелы, точки
...
# соавторы, у автора — много книг. `position` хранит ПОРЯДОК соавторов: на
# обложке он осмысленный, и терять его при разборе строки не хочется ---
class BookAuthor(SQLModel, table=True):
    __table_args__ = (
        UniqueConstraint("book_id", "author_id", name="uq_bookauthor_book_author"),
    )
    id: Optional[int] = Field(default=None, primary_key=True)
    book_id: int = Field(foreign_key="book.id", index=True, ondelete="CASCADE")
    author_id: int = Field(foreign_key="author.id", index=True, ondelete="CASCADE")
    position: int = 0                      # 0 — первый на обложке
