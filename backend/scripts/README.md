...
| `morning_check.py` | **Проверки начала дня:** документы, git-состояние (незакоммиченное + не выложенное), свежесть бэкапов, жив ли прод. `--offline` — без сети. Ничего не меняет |
| `docs_audit.py` | Проверка `docs/` на расхождение: похожие абзацы, битые перекрёстные ссылки, задачи закрытые в хронике но живые в бэклоге, разделы сверх лимита. Выдаёт **кандидатов**, решает человек. `--all` — полный список. Ничего не меняет |
...
| `sync_music_to_prod.py` | Точечный перенос подборок AI дев→прод: музыка и паспорта (`--export --since=` / `--import=`, `--category=music\|design`). Имя историческое — начинался с музыки |
| `regenerate_music.py` | Пересборка музыки пачкой книг для замера разнообразия (`--limit`, `--pause`, `--dry-run`). Книги витрины исключаются всегда |
| `explore_avoid.py` | Разбор молчащего `avoid` (з.99): форма payload, счётчик повторов без порога и то, что реально возвращает `_overused_items`. Ничего не меняет |
| `explore_palettes.py` | Разведка палитр паспортов: сходится ли модель в одну гамму, есть ли повторы цветов и шрифтов (`--csv`). Ничего не меняет |
