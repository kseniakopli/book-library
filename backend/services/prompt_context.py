...
#
# ⚠ Зачем отдельно от `_item_key`. До з.129 у аромата было одно поле `title`
# («Сандал»), и ключ по нему работал. После з.129 `title` — поэтичный образ,
# УНИКАЛЬНЫЙ по построению («Зелёная тяжесть зарослей»), а сырьё переехало
# в `material`. Ключ по title перестал совпадать хоть у кого-нибудь: порог
# AVOID_MIN_BOOKS не достигался никогда, `avoid` для ароматов молча опустел,
# и mode collapse остался без тормоза. Замер 12.08 после перегенерации
# 11 книг: ирис, ладан и дубовый мох — в 10 подборках из 22 каждый.

# Слова, которые называют ЧАСТЬ растения или форму, а не само сырьё.
# «ирис», «ирисовый корень» и «ирис корень» — одно и то же (з.135).
_PART_WORDS = {
    "корень", "корни", "лист", "листья", "листа", "семена", "семя",
    "цедра", "кожура", "трава", "травы", "цвет", "цветы", "цветки",
    "бобы", "боб", "кора", "почки", "смола", "абсолю", "масло",
    "экстракт", "эфирное", "сушеный", "сушеная", "молотый", "молотая",
    "гидролат", "настойка",
}

# Окончания прилагательных: «ирисовый» → «ирис», «перечная» → «переч».
# Список короткий намеренно: агрессивное усечение склеивает разные слова,
# а ложное слияние здесь тише ложного разделения — оно молча запретит
# сырьё, которого модель не предлагала.
_ADJ_SUFFIXES = ("овый", "евый", "иный", "ный", "ная", "ное", "ные",
                 "ский", "ская", "ой", "ый", "ая", "ое", "ые", "ий")
...
def _stem(word: str) -> str:
    ...
    # Родительный падеж: «семена кориандра» → «кориандр», «гидролат розы» →
    # «роза». Порог 4 подобран прогоном по 58 названиям из замера 12.08:
    # при 7 и 5 «роза» и «розы» оставались разными, при 4 склеились, и ни
    # одной ЛОЖНОЙ склейки не появилось («амбра»/«амбретта»,
    # «роза»/«розмарин» остались разными). Меняя порог — перепроверить тем же
    # прогоном, а не на глаз.
    if len(word) >= 4 and word[-1] in "аыия":
        return word[:-1]
    return word


def material_key(material: str) -> str:
    """Ключ тождества сырья: «ирис» == «ирисовый корень» == «ирис корень».

    Слова сортируются, поэтому «тонка бобы» и «бобы тонка» — одно.
    ё/е схлопывается: «чёрный чай» и «черный чай» тоже (та же болезнь,
    что в з.122 — гомоглифы в ключах тождества).
    """
    text = (material or "").lower().replace("ё", "е")
    text = re.sub(r"\([^)]*\)", " ", text)          # «водоросли (абсолю)»
    words = re.findall(r"\w+", text)
    kept = [_stem(w) for w in words if w not in _PART_WORDS]
    if not kept:                                     # осталось одно «корень»
        kept = [_stem(w) for w in words]
    return " ".join(sorted(set(kept)))


def avoid_key(category: str, item: dict) -> tuple[str, str]:
    """Отображаемое имя и ключ повтора для одного пункта подборки.

    ⚠ ОДНО место на продакшен и на разведку. `explore_avoid.py` держал свою
    копию этого правила, и после правки ключа его отчёт показывал старую
    картину (02.08). Для музыки это уже свернули в `track_key`; ветка
    еды/ароматов оставалась продублированной до 12.08.
    """
    if category == "music":
        return track_key(item.get("artist", ""), item.get("title", ""))
    if category == "aroma":
        # ⚠ Откат на title обязателен: в базе 272 пункта, сделанных
        # до з.129, — у них поля `material` нет вовсе.
        material = (item.get("material") or "").strip()
        if material:
            return material, material_key(material)
    name = (item.get("title") or "").strip()
    return name, _item_key(name)
