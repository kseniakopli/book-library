...
# --- Задача 103: режим «только чтение» для локальной разработки ---
# Дев и прод ходят в ОДИН аккаунт Spotify (`nctrnlib`): второго нет, потому что
# правила февраля 2026 дают пять мест на приложение. Плейлисты пересобираются
# по месту (`replace_playlist_items` меняет содержимое, ссылка остаётся), и
# локальная генерация музыки МОЛЧА переписывала прод-плейлисты: на странице
# книги в проде оставался старый список треков, а во встроенном плеере уже
# играл новый. Обнаружено 02.08.
#
# `SPOTIFY_READONLY=1` запрещает ЗАПИСЬ (создание плейлиста, замену треков,
# загрузку обложки). Чтение — поиск треков и резолв — работает как обычно,
# иначе генерация музыки лишилась бы проверки в Spotify и начала бы сохранять
# выдуманные треки.
#
# Читаем env при каждом вызове, а не при импорте: .env подхватывается при
# СТАРТЕ процесса, и переменная, выставленная в уже запущенном uvicorn,
# иначе не действовала бы (те же грабли, что с ALLOW_DEV_LOGIN).
def readonly() -> bool:
    return os.getenv("SPOTIFY_READONLY") == "1"
