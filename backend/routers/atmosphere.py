...
async def generate_atmosphere(
    ...
    #
    # Права (решение Ксении 02.08, по жалобе тестировщика). Атмосфера общая
    # для книги, но «создать с нуля» и «переписать готовую» — разные действия:
    #   • подборок ещё нет → генерировать может ЛЮБОЙ вошедший. Чужого он
    #     не портит: до него здесь было пусто;
    #   • подборки есть → только admin. Перегенерация меняет атмосферу для всех,
    #     включая витринные книги, чьи плейлисты уехали в печатные QR.
    #
    # ⚠ Раньше admin требовался всегда (решение 18.07), и это создавало тупик:
    # музыка/угощения/ароматы фоном не генерируются вовсе, а книга из каталога
    # без атмосферы (например, заведённая импортом CSV) оставалась пустой
    # навсегда — кнопки у тестировщика нет, эндпоинт отвечает 403.
    #
    # Расход держат общие лимиты (rate_limit.py, 20 AI-запросов в час на IP).
    with Session(database.engine) as session:
        book = get_book_or_404(session, book_id, lang)
        if read_selections(session, book_id, category):
            require_admin(session, lang, user_id)
