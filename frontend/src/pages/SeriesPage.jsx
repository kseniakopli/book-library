...
function SeriesPage() {
  ...
  // ⚠ `exact: true` (правка 04.08): `keys.series` = ["series"] — это ПРЕФИКС
  // ключа ["series", id], и без него строка выше обесценивалась собственной
  // же инвалидацией: карточка помечалась устаревшей и перезапрашивалась.
  // Заметно не было — повторный запрос возвращает то же самое, — но каждое
  // действие стоило лишнего похода на сервер. Нашлось на такой же паре
  // у автора (з.123), где мок в тесте не знал о новой книге и разница вылезла.
  const refresh = (fresh) => {
    queryClient.setQueryData(keys.seriesOne(id), fresh);
    queryClient.invalidateQueries({ queryKey: keys.series, exact: true });
  };
