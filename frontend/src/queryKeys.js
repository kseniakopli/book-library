...
//
// ⚠ Обратная сторона префиксов (найдено 04.08, задача 123): у пар вроде
// `authors` / `author(id)`, `series` / `seriesOne(id)`, `genres` / `genre(id)`
// короткий ключ ЯВЛЯЕТСЯ префиксом длинного. Поэтому
// `invalidateQueries({ queryKey: keys.authors })` помечает устаревшей
// и карточку автора. Само по себе это безвредно, но рядом с
// `setQueryData(keys.author(id), fresh)` инвалидация отменяет только что
// положенные данные: карточка перезапрашивается. На живом бэкенде повтор
// возвращает то же самое, и баг молчит.
// *Правило:* положил ответ мутации в кэш длинным ключом — инвалидируй
// короткий с `exact: true`.
export const keys = {
